...
size_t partition(size_t ary[], const size_t lm, const size_t rm)
{
    ...
    size_t i = lm - 1;                // Set the tracking index based on the leftmark (lm).
...
void quick_sort(size_t ary[], const size_t lm, const size_t rm)
{
    if (lm < rm) // Check if the leftmark is smaller than the rightmark.
    {
        ...
        quick_sort(ary, lm, pivot - 1);        // Recursively quicksort the elements before the pivot.
        quick_sort(ary, pivot + 1, rm);        // Recursively quicksort the elements after the pivot.
    }
}